...
- `npm run devnet:inspect:protocol -- --env .env.devnet.example` is available to read and validate the deployed program, protocol config, reward config, and reward vault state accounts before any public preview or wallet execution review. It also requires explicit independent treasury configuration and blocks admin/treasury address reuse.
